...
instance=N4MWC.GRFCalculations_ld_50_15()
othervars=np.array([0]) #no effect

def func(pars):
    out=[None,None]
    instance.fill_num_den(pars,othervars)
    result=instance.interfaceps(minx0=True,maxx1=True,absder=True)
    if result[0]>0:
...
#extremespars=[[1e-3,1e3],[1e-5,1e5],[1e-7,1e7],[1e-9,1e9],[1e-10,1e10]]
extrp=[1e-10,1e10]
...
matfile=os.path.join(outfolder_final,'mat_'+name+'.npy')
#print(outfolder)
if not os.path.exists(matfile):
    mat=[x for x in os.listdir(outfolder) if 'mat_N4MWC' in x][0]
    #print(mat)
    mat_pars=[x for x in os.listdir(outfolder) if 'mat_pars_N4MWC' in x][0]
    #print(mat_pars)
    mat=np.load(os.path.join(outfolder,mat))
    mat_pars=np.load(os.path.join(outfolder,mat_pars))
    settings["mat"]=mat
    settings["mat_pars"]=mat_pars
    done=False
else:
    done=True
