...
nrowm=41
nt=3*12*nrowm*3
grididx=int(jid//(nt/3))+1
matidx=(jid//(41*3))%12
row=(jid//3)%nrowm
colchunk=jid%3
c0,c1=[[0,27],[27,54],[54,82]][colchunk]

folder="/n/scratch3/users/r/rm335/2021_12_universalb/2022_07_12_N4av_noneq_b_g%d/N4avnoneq_out_%d"%(grididx,matidx)
matf=[x for x in os.listdir(folder) if x.startswith("mat") and not "pars" in x][0]
matparsf=[x for x in os.listdir(folder) if x.startswith("mat_pars")][0]

print(folder)
print(matf)
mat=np.load(os.path.join(folder,matf))
mat_pars=np.load(os.path.join(folder,matparsf))
